...
const getAllMitraNeedVerification = (status) => {
    const SQLQuery = `SELECT 
                        partners.partner_uid, 
                        partners.verified_status_uuid, 
                        partner_roles.partner_role_name as role,
                       ...
                       FROM partners
                       INNER JOIN partner_roles ON partners.partner_role_uuid = partner_roles.partner_role_uuid
                       INNER JOIN verified_status ON partners.verified_status_uuid = verified_status.verified_status_uuid
                       WHERE verified_status.verified_status LIKE '%${status}%' AND verified_status.nik IS NOT NULL`;
    return dbPool.execute(SQLQuery);
}

const registerPartners = async (uid, email, phone_number, username, uuid, defaultRole, domicile_address) => {    
    const SQLQuery1 = `INSERT INTO verified_status (verified_status_uuid, verified_status, message) VALUES (?, ?, ?)`
    ...
    const values1 = [uuid, "disabled", "Belum verifikasi KTP, silahkan verifikasi terlebih dahulu"]
    const values2 = [
        uid,
        uuid,
        email,
...
const updateStatusMitra = (body, updated_at, verified_status_uuid) => {
    const setClauses = [];
    ...
    const SQLQuery = `UPDATE verified_status 
                      SET ${setClauses.join(', ')}, verified_at="${updated_at}"
